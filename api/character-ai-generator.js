...
async function generateCharacterPromptWithGroq(characterData, model = 'llama-3.1-8b-instant', style, length, systemPrompt) {
  ...
  // 캐릭터 데이터를 전체 필드로 제공 (필터링 없음)
  const characterSummary = createFullCharacterData(characterData);

  const userPrompt = `내 친구를 소개해줄게! 이 친구 정말 매력적인 사람이야.

${characterSummary}

친구처럼 자연스럽게 소개해주세요:
- **편안한 대화체로 친구를 소개하는 느낌**으로 작성해주세요
- **이름을 반복하지 말고** "이 친구는", "그녀는", "그런데" 같은 자연스러운 표현 사용
- **'얘' 대신 '그녀'로 표현**해주세요 (더 우아하고 자연스럽게)
- **영어 단어 절대 사용 금지** (MBTI, ENFP, style 등 모든 영어 단어를 한국어로 변환)
- **수치 점수 언급 금지** (감정지능 7점, 자신감 10점 등 숫자로 설명하지 마세요)
- **대신 자연스러운 특징 묘사**로 ("정말 눈치가 빨라", "자신감이 넘쳐", "약간 신비로운 면이 있어")
- **필드명 대신 자연스러운 문맥 표현 사용**:
  * "대화역학이 재미있어" ❌ → "대화해보면" ✅
  * "외모적 특징은" ❌ → "생긴 건" ✅
  * "성격적으로는" ❌ → "성격을 보면" ✅
  * "취미 활동으로는" ❌ → "평소에 좋아하는 건" ✅
- **친구의 모든 매력과 특징을 빠짐없이 포함**하되 자연스럽게 녹여서 설명
- 외모, 성격, 취미, 대화 스타일, 연애 스타일, 좋아하는 것들 모두 포함
- **100% 순수 한국어로만 작성, 영어/한자 문자 절대 사용 금지**
- **친근하고 재미있게** 1500-2000자 정도로 소개
- "~야", "~지", "~거든" 같은 친근한 말투 사용

예시 시작: "내가 소개해줄 친구는 정말 특별한 사람이야. 그녀는..."로 시작하는 식으로 작성해주세요.`;

  try {
    // 타임아웃을 위한 AbortController 설정 (8초) - 더 상세한 프롬프트용
    ...
    const response = await fetch('https://api.groq.com/openai/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          {
            role: 'system',
            content: systemPrompt || '당신은 친구를 자연스럽게 소개하는 전문가입니다. 마치 가까운 친구가 다른 친구를 소개하듯이, 편안하고 친근한 말투로 매력적인 AI 소개글을 작성해주세요. 딱딱한 설명보다는 자연스러운 대화체로, 100% 순수 한국어로만 작성하며 영어 단어와 한자 문자는 절대 사용하지 마세요. "그녀"라는 표현을 사용하고, 필드명 대신 자연스러운 문맥 단어를 사용해주세요.'
          },
          {
            role: 'user',
            content: userPrompt
        ...
        max_tokens: length === 'short' ? 800 : length === 'medium' ? 1200 : 1800,
        temperature: 0.7
      })
    });
