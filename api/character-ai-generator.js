...
async function generateCharacterPromptWithGroq(characterData, model = 'llama-3.1-8b-instant', style, length, systemPrompt) {
  ...
  const userPrompt = `다음 캐릭터 데이터를 바탕으로 상세한 캐릭터 프롬프트를 작성해주세요.

${characterSummary}

중요한 요구사항:
- **반드시 한국어로만 답변**하세요 (영어 단어 사용 금지)
- **한자어 사용 금지** - 순우리말과 쉬운 한국어만 사용하세요
- **어려운 한자어 대신 쉬운 우리말 사용** (예: "심리적" → "마음의", "감정적" → "마음이", "성격적" → "성품이")
- **모든 제공된 필드와 값을 빠짐없이 포함**해야 합니다
- 수치 데이터 (감정지능, 자신감, 미스터리, 성적 호기심 등)를 정확히 반영
- 외모 세부사항 (가슴, 허리/힙, 스타일, 관능적 습관, 바디랭귀지) 포함
- 마음의 깊이 (핵심 욕구, 취약점, 가치관, 경계선) 상세히 반영
- 대화 방식의 모든 요소 (반응 성향, 말 습관, 어휘 수준) 포함
- 과거 이력 (연애 경험, 선호 스킨십, 첫 경험) 자연스럽게 포함
- 선호 선물과 미래 목표까지 언급
- **모든 영어 키워드를 적절한 한국어로 번역하여 사용**
- **쉽고 자연스러운 한국어로 매력적으로 작성**
- 1500-2500자 분량으로 충분히 상세하게
- **최종 결과는 100% 순우리말 한국어로 작성**되어야 함`;
    ...
    const response = await fetch('https://api.groq.com/openai/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          {
            role: 'system',
            content: systemPrompt || '당신은 한국어 AI 캐릭터 소개글 만들기 전문가입니다. 주어진 캐릭터 정보를 바탕으로 쉽고 자연스러운 한국어로 매력적인 캐릭터 소개글을 만들어주세요. 모든 답변은 반드시 순우리말 한국어로 해야 하며, 어려운 한자어는 사용하지 마세요.'
          },
          {
            role: 'user',
            content: userPrompt
