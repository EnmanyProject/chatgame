...
## ⚠️ 필수 비용 절감 지침 (반드시 준수)
### Claude를 시작할 때마다 이 지침을 따르세요:
1. **절대 HTML 파일 전체를 읽지 마세요** (특히 게임 대사 부분)
2. **구조 파악은 `dir`, `git status`, `git log`만 사용**
3. **파일 읽기 시 `limit` 파라미터 사용** (최대 50-100줄)
4. **검색이 필요하면 `Grep` 사용** (Read 대신)
5. **이미 캐시된 내용 재활용**
