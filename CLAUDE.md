...
- 📋 **해결 방법 안내**:
  * 시나리오 길이를 "짧음"으로 선택
  * gpt-3.5-turbo 모델 사용 (가장 빠름)
  * 잠시 후 다시 시도
- 🛡️ **JSON 파싱 에러 방지**: 응답이 JSON이 아닐 때 (HTML 에러 페이지) 파싱 시도 전에 감지
- 🔍 **에러 메시지 개선**: HTML 에러 응답을 텍스트로 읽어 정확한 에러 내용 표시
- 🎯 **근본 원인**: Vercel 서버리스 함수 10초 제한 vs OpenAI API 응답 시간

**기술적 개선**:
- Lines 20949-20952: 504 상태 코드 전용 에러 처리 추가
- Lines 20954-20958: response.ok 체크 후 JSON 파싱으로 안전성 강화
- 사용자 친화적인 에러 메시지로 UX 향상

**Git**: 커밋 `a235f99`, 푸시 완료 ✅
**영향**: scenario-admin.html Lines 20949-20958

---

### v1.19.1 (2025-10-09) - AI 생성 스토리 편집 기능 추가 (Patch Update)
**작업 내용**:
- 📝 **편집 가능화**: AI 생성 스토리 textarea를 readonly에서 편집 가능으로 변경
- 🎨 **UI 개선**:
  * 배경색: #f8f9fa → #ffffff (편집 가능 강조)
  * border: dashed → solid (일반 입력 필드처럼)
  * 라벨: "(자동 생성됨)" → "(편집 가능)"
  * placeholder 업데이트: 편집 가능 안내 추가
- 💾 **저장 로직**: 기존에 이미 aiGeneratedStory 저장 중 (line 9754, 9800)
- ✨ **사용자 경험**: AI 생성 후 내용을 자유롭게 수정하고 저장 가능

**Git**: 커밋 `161e881`, 푸시 완료 ✅
**영향**: scenario-admin.html Line 9754, 9800

---

### v1.19.0 (2025-10-09) - AI 프롬프트를 메신저 대화 소재 중심으로 수정 (Minor Update)
**작업 내용**:
- 🎯 **소설 → 단막극 스타일**: ai-prompts.json v1.6.0으로 업데이트
- 📖 **구조 생성 프롬프트**:
  * 단막극 스타일 강조
  * 짧은 대화 소재 중심
  * 각 단계 summary 2-3문장 → 1-2문장으로 간결화
- 📝 **스토리 생성 프롬프트**:
  * 목표 길이: 600-900자 → 400-600자로 단축
  * 완결형 소설 → 10-20분 안에 끝나는 짧은 메신저 대화 배경
  * 일상 소재 강조: "어제 그 일", "지금 상황", "내일 계획"

**Git**: 커밋 `7ebffd9`, 푸시 완료 ✅
**영향**: data/ai-prompts.json v1.6.0, scenario-admin.html v1.19.0

---

### v1.18.0 (2025-10-09) - 섹시 레벨 AI 프롬프트 반영 + 시나리오 설명 필드 순서 변경 (Minor Update)
**작업 내용**:
- 🔞 **섹시 레벨 5단계 지시문 추가**:
  * 1-2: 순수한 로맨스
  * 3-4: 일반적인 로맨스
  ...
  * 7-8: 관능적인 로맨스
  * 9-10: 매우 관능적인 로맨스
- 🤖 **AI 생성 반영**: API에서 섹시 레벨을 프롬프트에 포함하여 AI 생성 시 반영
- 🎨 **UX 개선**: 시나리오 설명 칸을 제목 바로 다음으로 이동
- 🔧 **함수 구현**: `getSexyLevelInstruction()` 함수 추가

**Git**: 커밋 `b6cb41b`, 푸시 완료 ✅
**영향**: api/scenario-manager.js, scenario-admin.html

---

### v1.17.1 (2025-10-09) - JSON 파싱 오류 수정 (Patch Update)
**작업 내용**:
- 🐛 **JSON 이스케이프 처리**: 올바른 이스케이프 처리로 파싱 오류 해결
- ✅ **안정성 개선**: JSON 생성 시 특수 문자 처리 강화

**Git**: 커밋 `c71a140`, 푸시 완료 ✅

---

### v1.17.0 (2025-10-09) - 시나리오 시간 개념 확장 및 구조 상세화 (Minor Update)
**작업 내용**:
- ⏰ **시간 개념 확장** (과거/현재/미래 모두 지원):
  * 이전: "이미 벌어진 일" (과거만 가능)
  * 현재: 과거/현재/미래 모두 시나리오 소재로 활용 가능
  * 예시:
    - 과거: "어제밤 키스했다" → 그 일에 대해 대화
    - 현재: "지금 데이트 중" → 현재 진행 상황 대화
    - 미래: "내일 고백할 예정" → 앞으로 일어날 일 대화

- 📖 **기승전결 구조 상세화** (1-2문장 → 2-3문장):
  * summary 필드가 더 구체적이고 풍부하게 생성됨
  * 상황과 감정을 더 깊이 있게 서술
  * AI가 더 상세한 구조를 만들어냄

**Git**: 커밋 `5dfc552`, 푸시 완료 ✅
**영향**: data/ai-prompts.json v1.5.0

---

### v1.16.0 (2025-10-09) - AI 프롬프트 어드민 편집 기능 완전 연동 (Minor Update)
**작업 내용**:
- ✨ **기능 완성**: 어드민 화면의 "AI 프롬프트" 탭에서 프롬프트를 수정하면 ai-prompts.json 파일에 즉시 반영

- 🔧 **구현 내용**:
  1. **프롬프트 로드**: ai-prompts.json 파일에서 실시간 로드
     - 기승전결 구조 생성 프롬프트
     - 장문 스토리 생성 프롬프트
     - 분위기 설정 (tone_settings) 유지

  2. **프롬프트 저장**: GitHub API를 통해 즉시 저장
     - tone_settings 보존
     - OpenAI/Claude/Llama 모든 AI 모델 파라미터 보존
     - 마지막 업데이트 시간 자동 갱신

  3. **데이터 구조 호환**:
     - parameters.openai/claude/llama 멀티 AI 모델 지원
     - emotion_flows 감정 흐름 패턴 유지

- 📝 **사용 방법**:
  1. AI 프롬프트 탭 열기
  2. "🔄 프롬프트 불러오기" 클릭
  3. System Prompt 또는 User Prompt Template 수정
  4. "💾 프롬프트 저장" 클릭
  5. 즉시 시나리오 생성에 반영됨

**Git**: 커밋 `b7e7eb0`, 푸시 완료 ✅

---

### v1.15.0 (2025-10-09) - 시나리오 생성 정확도 대폭 개선 (Minor Update)
**작업 내용**:
- 🎯 **문제 해결**:
  1. **제목/설명 무시 문제**: 프롬프트에 강력한 경고 추가
     - "🎯 필수: 반드시 아래 시나리오 정보를 정확히 따라야 합니다"
     - "⚠️ 중요: 위 제목과 설명의 내용을 절대 벗어나지 마세요"
     - 예시 삭제하여 혼란 방지

  2. **시간 배경 고정 문제**: 동적 시간 처리
     - "어제밤" 하드코딩 제거
     - "설명"에 명시된 시간 배경 사용 (어제밤/방금 전/한 달 전 등)

  3. **가벼움 분위기 미적용**: 분위기 지시 강화
     - 분위기 instruction을 프롬프트 **맨 앞**에 배치 (최우선 적용)
     - "가벼움" 모드에 금지 표현 명시 (불안/두려움/갈등/고민/걱정 금지)
     - 긍정 감정만 사용 (설렘/기대감/즐거움/행복/두근거림/미소/웃음)

**Git**: 커밋 `0cee067`, 푸시 완료 ✅
**영향**: data/ai-prompts.json v1.4.0, api/scenario-manager.js

---

### v1.14.1 (2025-10-09) - 로그인 시 401 에러 표시 개선 (Patch Update)
**작업 내용**:
- 🔧 **문제 해결**: 로그인 초기화 시 발생하는 401 에러를 정상 케이스로 처리
- 📝 **변경 사항**:
  * API 키 미등록 상태를 에러가 아닌 정보 로그로 표시
  * console.error → console.log/console.warn으로 변경하여 혼란 방지
  * scenario-admin.html: loadUserApiKey() 함수 개선
  * 401 응답 시 조기 리턴 및 정보 로그 출력

- 💡 **설명**: 로그인 직후 API 키 조회 시 401은 정상적인 흐름입니다. 사용자가 아직 API 키를 등록하지 않은 상태를 의미하며, 기능상 문제가 없으므로 에러가 아닌 정보로 표시합니다.

**Git**: 커밋 `5f6652e`, 푸시 완료 ✅

---

### v1.14.0 (2025-10-09) - 분위기 조절 기능 추가 - 3단계 시스템 (Minor Update)
**작업 내용**:
- 🎨 **분위기 조절 시스템 구현**:
  * data/ai-prompts.json: tone_settings 3단계 추가 (light/balanced/serious)
  * scenario-admin.html: 분위기 선택 드롭다운 UI 추가
  * api/scenario-manager.js: 분위기 기반 프롬프트 및 temperature 조정 로직

- 📊 **각 분위기별 설정**:
  * 가벼움 (light): temp 0.9, 밝고 귀엽고 장난스러운 분위기
  * 보통 (balanced): temp 0.8, 진솔하면서도 따뜻한 분위기 (기본값)
  * 진지함 (serious): temp 0.7, 깊고 진지하며 성찰적인 분위기

**Git**: 커밋 `c553f4e`, 푸시 완료 ✅

---

### v1.13.1 (2025-10-09) - 시나리오 생성 에러 수정 (Patch Update)
**작업 내용**:
- 🐛 **문제**: selectedStructure가 null일 때 접근하여 TypeError 발생
- ✅ **해결**: AI 응답 및 Fallback 모두 실패 시 명확한 에러 메시지
- 📝 **개선**: API 응답 상세 로깅 추가
- 🔍 **디버깅**: 콘솔에 전체 에러 객체 출력

**Git**: 커밋 `2fed8f1`, 푸시 완료 ✅
...
## 📚 과거 작업 아카이브 (2025-01 ~ 2025-09)

> 상세한 이전 작업 히스토리는 Git 커밋 로그에서 확인 가능합니다.
> 주요 마일스톤만 요약합니다.

### 주요 마일스톤

#### 2025-09-28: 시스템 안정화 및 초기화
- 시나리오 중복 문제 해결
- GitHub API 완전 통합
- 데이터 동기화 시스템 구축

#### 2025-09-16: 채팅 훈련 시스템 전환
- MBTI 로맨스 게임 → 남성 채팅 기술 훈련 시스템으로 재정의
- OpenAI GPT-4 기반 AI 평가 시스템
- 4단계 난이도 × 5개 MBTI = 20가지 커리큘럼

#### 2025-09-02: 시나리오-에피소드 분리 시스템
- 36퀘스트 시스템 구현
- 시나리오(배경) ↔ 에피소드(대화) 분리 구조
- Claude 3.5 Sonnet 통합

#### 2025-08-30: 프로젝트 기반 구축
- Vercel 배포 환경 구축
- GitHub 자동 배포 파이프라인
- 기본 게임 시스템 구현
...
# 특정 기간 커밋 확인
git log --since="2025-09-01" --until="2025-09-30" --oneline

# 파일별 변경 히스토리
git log --follow --oneline -- scenario-admin.html
```

---

*마지막 업데이트: 2025-10-09*  
*최신 버전: v1.19.1 - AI 생성 스토리 편집 기능*
